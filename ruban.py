...
RECEIVER_EMAIL = os.getenv("RECEIVER_EMAIL", "rubanclement23@gmail.com")
...
def send_email_alert(detected_animals, image_path):
    ...
    try:
        ...
        with smtplib.SMTP_SSL('smtp.gmail.com', 465, context=context) as server:  # Using SSL instead of TLS
...
def detect_and_alert(source):
    ...
    # Perform prediction
    try:
        results = model.predict(source=source, show=False, save=True)  # Removed GUI dependency
        detected_animals = set()
        image_path = "detected_image.jpg"  # Path to save detected image

        for result in results:
            for cls in result.boxes.cls:
                animal_name = model.names[int(cls)]
                detected_animals.add(animal_name)
            result.save(filename=image_path)  # Save the detection image
...
if __name__ == "__main__":
    # Replace with a valid file path or `0` for webcam
    source_path = ('karunya_elphant.png')
    try:
